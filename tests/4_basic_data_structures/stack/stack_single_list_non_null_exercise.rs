use data_structures_and_algorithms_in_rust::_4_basic_data_structures::exercise::{
    ExerciseStack, Peek, SingleListHeadNonNullStack, SingleListTailNonNullStack,
};

#[test]
fn single_list_head_non_null_stack_test() {
    let mut s = SingleListHeadNonNullStack::new();
    s.push(1); // 1
    s.push(2); // 2 -> 1
    s.push(4); // 4 -> 2 -> 1
    assert_eq!(3, s.size());
...
#[test]
fn single_list_tail_non_null_stack_test() {
    let mut s = SingleListTailNonNullStack::new();
    s.push(1); // 1
    s.push(2); // 1 -> 2
    s.push(4); // 1 -> 2 -> 4
    assert_eq!(3, s.size());
